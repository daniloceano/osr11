# Step 3 — Tidal Sensitivity Analysis
...
```bash
conda run -n osr11 python src/03_tidal_sensitivity/main.py --all
```
...
conda run -n osr11 python src/03_tidal_sensitivity/main.py --event-figures

# Summary figures and tables only
conda run -n osr11 python src/03_tidal_sensitivity/main.py --summary
```
...
- **Reuses:** `src/02_preliminary_compound` — data loading, event record building, SSH-only thresholds
- **Feeds into:** `src/04_threshold_calibration` — same SSH_total definition (zos + FES2022 daily max tide) and same Hₛ daily-maximum convention
