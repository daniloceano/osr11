...
**Location:** `src/02_threshold_calibration/01_exploratory_data_analysis/`
...
```
src/02_threshold_calibration/01_exploratory_data_analysis/
├── __init__.py
...
> **Note on imports:** The physical directory is `src/02_threshold_calibration/01_exploratory_data_analysis/`
> but it is registered under `src.exploratory_data_analysis` (and the legacy alias `src.explore_test_data_south_sc`)
> via compatibility aliases in `src/__init__.py`. All `from src.exploratory_data_analysis.xxx import` statements
> work unchanged. Adding new numbered modules only requires updating the `_MODULE_ALIASES` dict
> in `src/__init__.py`.
