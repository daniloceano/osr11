...
def _get_sector_colors() -> dict[str, str]:
    ...
    try:
        from config.plot_config import SECTOR_COLORS
        return SECTOR_COLORS
    except ImportError:
        # Fallback matching config/plot_config.py defaults — only used if
        # config package is somehow unavailable (should never happen in OSR11).
        return {
            "North":          "#1f77b4",
...
# Hardcoded municipality → coastal_sector lookup derived from the events database.
# Used as a fallback when 'coastal_sector' is absent from the event_status_df
# (e.g., loaded from a tab_TC5_event_capture_status.csv generated by an older run).
_MUNICIPALITY_SECTOR: dict[str, str] = {
    "São Francisco do Sul": "North",
    "Joinville":            "North",
    "Barra Velha":          "North",
    "Balneário Camboriú":   "Central-north",
    "Itajaí":               "Central-north",
    "Porto Belo":           "Central-north",
    "Bombinhas":            "Central-north",
    "Florianópolis":        "Central",
    "Palhoça":              "Central",
    "Tijucas":              "Central",
    "Biguaçu":              "Central",
    "Imbituba":             "Central-south",
    "Laguna":               "Central-south",
    "Passo de Torres":      "South",
}


def _ensure_sector_column(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure the 'coastal_sector' column exists; derive from municipality if missing."""
    if "coastal_sector" not in df.columns or df["coastal_sector"].isna().all():
        df = df.copy()
        df["coastal_sector"] = df["municipality"].map(_MUNICIPALITY_SECTOR).fillna("Unknown")
        log.debug(
            "coastal_sector column derived from municipality lookup "
            "(re-run --scoring to get it from the events database directly)."
        )
    return df
...
def _require_ssh(df: pd.DataFrame, fig_id: str) -> pd.DataFrame:
    ...
    n_ssh_nan = df["peak_ssh_causal"].isna().sum()
    if n_ssh_nan == len(df):
        raise RuntimeError(
    ...
    if n_ssh_nan > 0:
        log.warning(
            "%s: %d/%d events have NaN peak_ssh_causal — dropping them.",
            fig_id, n_ssh_nan, len(df),
        )
        df = df.dropna(subset=["peak_ssh_causal"])
    return df
...
def plot_event_capture_scatter(
    ...
    """TC5-E1 — Peak Hₛ per event, sorted by coastal sector then date.

    Each point represents one event from the final combined positive set.
    Y = peak Hₛ within the causal window [D-2 … D+1].
    X = event rank (sorted by coastal sector, then date).
    Filled markers = captured (HIT) at the PU-optimal pair.
    Open markers   = missed.
    Colour encodes coastal sector (same SECTOR_COLORS as Step 2d).
    Dashed horizontal line = median local Hₛ threshold across all grid points.

    Parameters
    ----------
    event_status_df : DataFrame from scoring.get_event_capture_status().
        Required columns: peak_hs_causal, peak_ssh_causal, captured,
        coastal_sector, municipality, date, thr_hs, thr_ssh.
    optimal : dict with thr_hs_pct, thr_ssh_pct, H, M.
    output_path : where to save.
    """
    from matplotlib.lines import Line2D
    SECTOR_COLORS = _get_sector_colors()

    df = event_status_df.dropna(subset=["peak_hs_causal"])
    if df.empty:
        log.warning("TC5-E1: no valid peak_hs_causal values — skipping.")
        return
    df = _ensure_sector_column(df)

    opt_hs_pct = float(optimal.get("thr_hs_pct", 0.90))
    H = int(df["captured"].sum())
    M = len(df) - H

    # Sort by coastal sector (canonical order) then date
    sector_order = list(SECTOR_COLORS.keys())
    df = df.copy()
    df["_sector_rank"] = df["coastal_sector"].apply(
        lambda s: sector_order.index(s) if s in sector_order else len(sector_order)
    )
    df_sort = df.sort_values(["_sector_rank", "date"]).reset_index(drop=True)
    ...
    if not np.isnan(median_thr_hs):
        ax.axhspan(
            median_thr_hs, df_sort["peak_hs_causal"].max() * 1.10,
    ...
    for sector, color in SECTOR_COLORS.items():
        df_s = df_sort[df_sort["coastal_sector"] == sector]
        if df_s.empty:
            continue
        ...
        if not miss.empty:
            ax.scatter(
                miss.index, miss["peak_hs_causal"],
                facecolors="none", edgecolors=color, linewidths=1.5,
                s=60, zorder=2,
            )
        if not hit.empty:
            ax.scatter(
                hit.index, hit["peak_hs_causal"],
                facecolors=color, edgecolors=color, linewidths=0.7,
                s=60, zorder=3,
            )
    ...
    sector_handles = [
        Line2D([0], [0], marker="o", linestyle="none",
               ...
               markersize=7, label=sector)
        for sector, color in SECTOR_COLORS.items()
        if not df_sort[df_sort["coastal_sector"] == sector].empty
    ]
    marker_handles = [
        Line2D([0], [0], marker="o", linestyle="none",
    ...
    first_legend = ax.legend(
        handles=sector_handles, title="Coastal sector", fontsize=8,
    ...
    ax.set_title(
        f"TC5-E1 — Peak Hₛ per event at PU-optimal pair (Hₛ q{round(opt_hs_pct*100)})\n"
...
def plot_ssh_capture_strip(
    ...
    ----------
    event_status_df : DataFrame from scoring.get_event_capture_status().
        Required columns: peak_ssh_causal, captured, coastal_sector, thr_ssh.
    optimal : dict with thr_hs_pct, thr_ssh_pct, H, M.
    output_path : where to save.
    """
    from matplotlib.lines import Line2D
    ...
    df = _require_ssh(event_status_df, "TC5-E2")
    df = _ensure_sector_column(df)
    ...
    median_thr_ssh = df_sort["thr_ssh"].median()

    fig, ax = plt.subplots(figsize=(10, 5))

    if not np.isnan(median_thr_ssh):
        ax.axhspan(
            median_thr_ssh, df_sort["peak_ssh_causal"].max() * 1.10,
            color="#d0e8f8", alpha=0.20, zorder=0,
        )
        ax.axhline(
    ...
    for sector, color in SECTOR_COLORS.items():
        ...
        if not miss.empty:
            ax.scatter(
                miss.index, miss["peak_ssh_causal"],
                facecolors="none", edgecolors=color, linewidths=1.5,
                s=60, zorder=2,
            )
        if not hit.empty:
            ax.scatter(
                hit.index, hit["peak_ssh_causal"],
                facecolors=color, edgecolors=color, linewidths=0.7,
                s=60, zorder=3,
            )
...
def plot_peak_scatter_pu(
    ...
    df = _require_ssh(event_status_df, "TC5-E3")
    df = _ensure_sector_column(df)
    ...
    median_thr_hs  = df["thr_hs"].median()
    median_thr_ssh = df["thr_ssh"].median()

    fig, ax = plt.subplots(figsize=(9, 7))

    # Light shading: zone where both median thresholds are exceeded
    xlim_hi = df["peak_hs_causal"].max() * 1.08
    ylim_hi = df["peak_ssh_causal"].max() * 1.08
    if not (np.isnan(median_thr_hs) or np.isnan(median_thr_ssh)):
        if xlim_hi > median_thr_hs and ylim_hi > median_thr_ssh:
            ax.fill_betweenx(
    ...
    for sector, color in SECTOR_COLORS.items():
        df_s = df[df["coastal_sector"] == sector]
        if df_s.empty:
            continue
        miss = df_s[~df_s["captured"]]
        hit  = df_s[ df_s["captured"]]
    ...
    sector_handles = [
        ...
        for sector, color in SECTOR_COLORS.items()
        if not df[df["coastal_sector"] == sector].empty
    ]
    ax.legend(
...
def run_all_figures(
    ...
    event_status_df : optional DataFrame from scoring.get_event_capture_status().
        When provided, generates event-level capture figures (TC5-E1/E2/E3).
    ...
    if _event_status is not None and not _event_status.empty:
        # TC5-E1 — peak Hₛ per event sorted by sector (no SSH required)
        ...
        try:
            plot_ssh_capture_strip(
                _event_status, optimal,
                summary_dir / "fig_TC5_E2_ssh_capture.png",
            )
        ...
        try:
            plot_peak_scatter_pu(
                _event_status, optimal,
                summary_dir / "fig_TC5_E3_peak_scatter.png",
            )
        except RuntimeError as e:
            log.error("TC5-E3 skipped: %s", e)
    else:
        log.warning(
            "Event capture status not available — skipping TC5-E1/E2/E3. "
