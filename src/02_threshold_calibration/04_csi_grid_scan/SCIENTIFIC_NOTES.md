# SCIENTIFIC_NOTES — Step 2d: Threshold Calibration (CSI Grid Scan)

**Part of:** Step 2 — Threshold Calibration (umbrella step)

**Module**: `src/02_threshold_calibration/04_csi_grid_scan/`
...
Zscheischler et al. (2020) and is consistent with Steps 2a–2c of this pipeline.
...
the `eo-tides` Python library (see Step 2c documentation).
...
### Step 2d.0 — Reuse of infrastructure from Steps 2a–2c

The following components are reused **without modification**:

- `src/02_threshold_calibration/02_preliminary_compound/events.build_event_records()`: resolves each of the 91
...
- `src/02_threshold_calibration/03_tidal_sensitivity/tides.build_tide_cache()` (Step 2c): computes FES2022 tidal predictions
  at daily 00:00 UTC for each unique grid point.
- `src/02_threshold_calibration/03_tidal_sensitivity/tides.add_tide_to_ssh()` (Step 2c): computes SSH_total = SSH + tide.

### Step 2d.0b — Temporal domain restriction (preprocessing)
...
**Implementation:** `src/02_threshold_calibration/04_csi_grid_scan/preprocessing.py`,
...
### Step 3b.1 — Causal/antecedent matching window
...
### Step 3b.2 — Threshold grid scan (Layer 1: hits and misses)
...
### Step 3b.3 — Full-series false alarm detection (Layer 2)
...
### Step 3b.4 — Metric computation and optimal pair selection
...
2. **Local percentile thresholds**: Thresholds are computed from the clipped annual
   ...
   Step 2 and sub-step 3a and keeps the comparison clean.
