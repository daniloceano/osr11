# Step 2d — CSI Grid Scan: Quick-Start Guide
...
This runs:
1. FES2022 tidal series computation (reuses Step 2c)
...
python -m src.csi_grid_scan.main --hits-misses

# Layer 2 only
python -m src.csi_grid_scan.main --false-alarms

# Summary only (requires previous layer outputs to be held in memory)
# Note: --all is the recommended entry point
python -m src.csi_grid_scan.main --summary
```
...
|-----------|-------|-----------|
| SSH variable | SSH_total = SSH + FES2022 | Consistent with Step 2c (tidal sensitivity) |
...
| Local thresholds | Full climatological series | Consistent with Steps 2b and 2c |

See `src/02_threshold_calibration/04_csi_grid_scan/SCIENTIFIC_NOTES.md` for full methodological documentation.
