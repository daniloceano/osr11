...
| **Status** | `executado-com-criterio-reprovado` — a adoção do portão HAT e a recalibração do Step 2e foram **executadas em 2026-07-30**. Duas das três consequências falsificáveis passaram e **a terceira, (c), reprovou**. A entrada de decisão determinava que, se (c) falhasse, "a decisão deve ser reaberta"; a decisão **não foi reaberta**, foi reafirmada pelo pesquisador responsável, **Danilo Couto de Souza**, e executada. O §6 do README da comparação, que diz que os resultados "não autorizam adotar HAT como método vigente", **permanece intocado e contrário à adoção**. Ver §14, entrada de execução de 2026-07-30 |
| **Desfecho** | `metodologia-alterada` — detector redesenhado em 2026-07-29 (maré como variável condicionante) e novamente em 2026-07-30 (portão e datum em HAT, par de limiares recalibrado sobre o detector de produção, q90/q90 → q70/q99). Fecha **em conjunto com AUD-06**, do qual é inseparável. **A afirmação de fechamento de 2026-07-29, "a maré não participa mais da decisão sobre a existência do evento", é forte demais**: vale para os percentis de detecção, não para o portão, que contém a maré. Sob HAT o portão continua contendo a maré, mas o termo astronômico do excesso passa a ser sempre ≤ 0 por definição. **A adoção agravou AUD-02**, que segue bloqueando publicação |
| **Depende de** | — |
...
| **Última atualização** | 2026-07-30 (execução da adoção e recalibração do Step 2e) |
...
| 2026-07-30 | _hash a registrar no commit_ | `main` | **Novos:** `src/exploratory/audit_AUD_01_hat_gate_sensitivity.py`, `outputs/audit/AUD-01_hat_gate_sensitivity/` | Diagnóstico read-only do poder informativo do portão de nível. **Nenhum arquivo do pipeline, do catálogo ou do índice foi alterado**; a questão passa a `reaberto` no cabeçalho |

| 2026-07-30 | `4724a07` e seguintes | `main` | **Novos:** `src/03_storm_catalog_generation/02_compound_detection/compound_core.py`, `src/exploratory/audit_AUD_01_step2e_score_surface.py`, `src/exploratory/audit_AUD_01_final_criteria.py`, `src/exploratory/audit_AUD_02_threshold_grid_floor.py`, `outputs/legacy_mhws_method/`, `outputs/legacy_threshold_calibration_ssh_total/`, `outputs/current_method_hat/`, `outputs/audit/AUD-01_step2e_score_surface/`, `outputs/audit/AUD-01_final_criteria/`, `outputs/audit/AUD-02_threshold_grid_floor/`. **Alterados:** `src/02_threshold_calibration/05_pu_composite_calibration/{config/analysis_config.py, scoring.py, audit.py, sensitivity.py, utils.py, main.py}`, `.../02_compound_detection/detection_hat.py`, `src/04_risk_integration/hazard_index.py`, `src/site/{select_timeseries_points.py, export_timeseries_panel_data.py}`, `site/components/CompoundTimeSeriesChart.tsx`, `site/lib/timeseriesData.ts`, `site/public/data/*`. **Intocados:** `detection_mhws.py`, `detection.py`, `mhws_datum.py`, o §6 de `outputs/method_comparison_mhws_vs_hat/README.md` e todo o histórico anterior da §14 | Execução da adoção do portão HAT, recalibração do Step 2e sobre o detector de produção e regeneração dos produtos publicados. Os dois legados foram versionados **antes** de qualquer escrita |
...
### 2026-07-30 — Poder informativo do portão de nível; sensibilidade a HAT e ao híbrido portão/datum

| Campo | Conteúdo |
|-------|----------|
| **Pergunta testada** | Levantada pelo usuário a partir do painel de séries temporais do site: se o MHWS "não está resolvendo o problema no Norte", (a) o portão `max(SWL) > MHWS` exige de fato a contribuição meteorológica, ou é satisfeito pela astronomia sozinha? (b) o que muda na detecção se o portão for o HAT? (c) é coerente usar HAT como portão e manter o MHWS como datum da severidade, para não perder o sinal astronômico na magnitude? Esta entrada ataca diretamente a **incerteza remanescente (2)** registrada no fechamento de 2026-07-29 — o travamento residual esperado no Norte pela condição de MHWS, "legítimo mas não medido" |
| **Dados e métodos** | Diagnóstico read-only sobre os 808 pontos, reaproveitando `compound_events_at_point` sem alteração. Três métricas novas: **(i) fração maré-sozinha** — para cada evento aceito, se `max(maré) > portão` nos mesmos dias de sobreposição, isto é, se o portão teria sido cruzado com `zos′ = 0`; **(ii) decomposição do excesso diário** pela identidade exata `SWL − portão = zos′ + (maré − portão)`, separando termo meteorológico e astronômico nos dias de critério pleno; **(iii) constante herdada do híbrido**, pela identidade `SWL − MHWS = zos′ + (maré − HAT) + (HAT − MHWS)`, cujo último termo é constante por ponto. HAT estimado como `max(tide_daily_max)` em 1993–2025 — 33 anos cobrem o ciclo nodal de 18,6 anos —, mesmo estimador do `max_HAT_estimate` do diagnóstico de datum de 2026-07-29 |
| **Scripts executados** | `conda run -n osr python -m src.exploratory.audit_AUD_01_hat_gate_sensitivity` |
| **Novas saídas geradas** | `outputs/audit/AUD-01_hat_gate_sensitivity/hat_gate_by_point.csv`, `hat_gate_by_band.csv`, `hat_gate_summary.json` |
| **Achados** | **(1) O portão MHWS é pouco informativo em toda a costa, não só no Norte.** A maré sozinha já cruzaria o portão em **73,0%** dos eventos ao norte de 15°S e em **79,6%** ao sul de 25°S (mediana do domínio 0,667). Por faixa: RS 0,966, AP 0,832, N equatorial 0,766, SC/PR 0,601, NE 0,584, BA-N 0,550, ES/BA-S 0,517, SP/RJ 0,385. Os dois extremos falham por motivos opostos: no Norte a sizígia cruza o MHWS rotineiramente; no RS o `MHWS = A_M2 + A_S2` vale 0,109 m e qualquer maré o cruza. **Isto não invalida os eventos** — a seleção `zos ≥ q90 ∩ Hs ≥ q90` continua garantindo coocorrência dos forçantes —, mas mostra que a terceira condição agrega pouca informação meteorológica. **(2) A severidade não é comparável entre regiões.** Nos dias de critério pleno o excesso médio decompõe-se em: AP 0,124 m meteorológico contra **0,156 m astronômico** (56% astronomia); N equatorial 0,094 / **0,129** (58%); RS 0,388 / 0,139 (26%); SP/RJ 0,214 / **−0,050** — negativo, ou seja, ali os eventos ocorrem com a maré *abaixo* do MHWS e a sobrelevação carregando o nível. Em **241 dos 808 pontos** o termo astronômico supera o meteorológico. **(3) Um portão em HAT é uma exclusão implícita de domínio.** Eventos no domínio caem de 79.639 para 37.225 (−53%); ao norte de 15°S, de 14.582 para **545** (−96,3%); **248 dos 808 pontos ficam com zero eventos**, concentrados em N equatorial (101), BA-N (57) e NE (49). Restam 0,04 evento/ponto/ano no Norte, insuficiente para sustentar uma componente de frequência. **(4) Sob HAT a maré deixa de contribuir magnitude, como previsto analiticamente.** Como `maré ≤ HAT` por definição, o termo astronômico é sempre ≤ 0: −0,095 m no Norte, −0,213 m no Sul. O excesso passa a ser a sobrelevação descontada do déficit de maré. A `fração maré-sozinha = 0` sob HAT é **tautologia, não evidência**. **(5) O híbrido portão-HAT/datum-MHWS é indefensável.** A constante herdada `HAT − MHWS` (0,571 m médios no Norte, 0,335 m no Sul) responde por **98,6% do excesso no NE, 94,3% no AP, 93,7% no N equatorial** e 67,6% no RS. A severidade viraria um número idêntico para todos os eventos de um mesmo ponto, determinado só pela estrutura harmônica local; sob normalização agrupada no domínio, isso empurra sistematicamente os pontos macromareais para cima — o modo de falha da formulação `A_absolute_total_level` (ρ(\|lat\|) = −0,847) reintroduzido por outra via |
| **Interpretação** | O achado (1) qualifica, sem anular, o fechamento de 2026-07-29. A correção adotada — retirar a maré dos percentis de detecção — continua válida e é o que produziu o gradiente S→N coerente. O que não foi verificado na ocasião é que a **terceira condição, que permaneceu dependente da maré, faz pouco trabalho meteorológico**: ela funciona como filtro de fase de sizígia no Norte e como quase no-op no Sul. O achado (2) é o mais consequente para o artigo, porque a severidade é componente do índice com peso 1/2 e seu conteúdo físico varia com a latitude — um mesmo valor significa astronomia no Amapá e sobrelevação em São Paulo. Os achados (3) e (5) fecham as duas saídas mais óbvias: o HAT não corrige o Norte, esvazia-o, e portão e datum não podem divergir, porque o excesso só tem interpretação como distância à condição que define o evento. A conclusão metodológica geral é que **portão e datum devem ser o mesmo nível**, e que a escolha desse nível é inseparável da questão do domínio de validade já levantada na partição de 2026-07-29 |
| **Alterações implementadas** | **Nenhuma.** Diagnóstico read-only. Pipeline, catálogo publicado, índice, site e figuras do artigo intocados. Nenhuma formulação adotada |
| **Validação realizada** | (1) A detecção usa `compound_events_at_point` sem modificação, de modo que o braço MHWS reproduz o catálogo publicado por construção — total de 79.639 eventos idêntico ao `compound_summary_mhws.json`. (2) As decomposições são identidades algébricas exatas, não ajustes: `SWL − portão ≡ zos′ + (maré − portão)` vale dia a dia por definição de `still_water_level`. (3) O estimador de HAT é o mesmo do diagnóstico de datum de 2026-07-29, permitindo comparação direta com aquele resultado (excesso médio ao norte de 15°S de −0,146 m sob `max_HAT_estimate`) |
| **Incerteza remanescente** | (1) **O teste de Rayleigh continua sem ser reexecutado sobre o catálogo final** — a fração maré-sozinha mede o poder informativo do portão, não a distribuição de fase dos eventos, e não substitui aquele diagnóstico. (2) O HAT foi estimado como máximo de uma amostra de 33 anos: é estatística de ordem extrema, dependente do comprimento do registro, ao contrário do `A_M2 + A_S2`, que é analítico — um portão em HAT tornaria o conjunto de eventos dependente da janela temporal e não transferível a projeções. (3) Portões intermediários (q95, q99 de `tide_daily_max`) **não foram testados** sobre o detector vigente; o diagnóstico de datum de 2026-07-29 os avaliou apenas como datum de severidade, sob o detector antigo. (4) A fração maré-sozinha é condicional aos candidatos `zos ∩ Hs`, portanto mede o trabalho do portão dado o par de forçantes, não a probabilidade incondicional de coocorrência. (5) AUD-02, AUD-04, AUD-12 e AUD-18 permanecem abertos e nada aqui os alcança |
| **Próxima decisão necessária** | **Três decisões separadas, em ordem de custo.** (a) Publicar a decomposição meteorológica/astronômica do excesso como diagnóstico por ponto e passar a interpretar a severidade por regime — não altera nenhum resultado e não regenera nada. (b) Rodar a varredura de portões q90/q95/q99/HAT sobre o detector vigente, para dispor da curva de compromisso entre exigir sobrelevação e preservar eventos, antes de qualquer mudança. (c) Decidir se o setor tide-dominated permanece no produto: se permanecer, a severidade precisa de ressalva regional explícita; se sair, deve sair pela partição declarada de 2026-07-29 (antimodo em 0,257; 268 contra 540 pontos; custo registrado de 66,8% dos pontos e 42,1% dos eventos), nunca por um limiar que a esvazie em silêncio |

### 2026-07-30 — Braço HAT completo e efeito componente a componente no índice

| Campo | Conteúdo |
|-------|----------|
| **Pergunta testada** | Mantendo detector, calibração, pesos e fórmula do índice literalmente iguais, o que muda quando portão **e** datum de severidade passam de MHWS para HAT? O efeito exploratório de contagem sobrevive à normalização própria do braço, à integração da severidade e ao risco municipal? |
| **Dados e métodos** | HAT = `max(tide_daily_max)` em 1993–2025 por ponto. Duas fases: detecção paralela dos 808 pontos com `compound_events_at_point`; barreira global com Q05/Q95 dos excessos do próprio braço; pontuação paralela. A ordem ponto→evento→dia foi fixada. Frequência e severidade integrada foram explicitamente zeradas onde não há evento, em ambos os braços, antes de chamar `derive_native_hazard_index()`, preservando a população comum de 808 pontos. Exposição, SVI e associação municipal vieram do produto publicado e são idênticas |
| **Scripts executados** | `python -m src.compound_detection.detection_hat --workers 8 --validate-points 30`; `python -m src.exploratory.compare_methods_mhws_vs_hat`; `python -m src.exploratory.plot_method_comparison_mhws_vs_hat` |
| **Novas saídas geradas** | Instantâneo versionado em `outputs/hat_method/`; comparação completa em `outputs/method_comparison_mhws_vs_hat/`, com CSVs, JSON, README e cinco mapas |
| **Achados** | **(1) Aceitação reproduzida exatamente:** 37.225 eventos no domínio, 545 ao norte de 15°S, 24.196 ao sul de 25°S e 248 pontos sem evento; `thr_hs` idêntico em 808/808 pontos. **(2) Referências próprias importam:** Q95 diário de nível cai de 0,7231 m (MHWS) para 0,4044 m (HAT); nenhum percentil MHWS foi reaproveitado. **(3) Componentes na grade:** Spearman MHWS×HAT = frequência 0,877, severidade 0,703, índice 0,894. ρ(\|lat\|, índice) sobe de +0,584 para +0,658. Médias do índice: RS 0,826→0,664; SC/PR 0,585→0,432; SP/RJ 0,454→0,327; ES/BA-S 0,255→0,116; BA-N 0,177→0,005; NE 0,125→0,004; N eq. 0,167→0,033; AP 0,288→0,060. **(4) Frequência e severidade tornam-se quase a mesma ordenação** sob HAT: ρ interno +0,599→+0,958, em parte porque compartilham a massa de 248 zeros. **(5) Risco municipal:** Spearman 0,700; sobreposição top-10 4/10; fração ao norte de 20°S 50%→0%; 96 municípios ficam associados a pontos sem evento. Apicum-Açu 1º→167º, Turiaçu 2º→169º e Macapá 16º→134º; Presidente Kennedy 236º→122º e Itajaí 230º→126º |
| **Interpretação** | O braço completo confirma e fortalece o diagnóstico anterior: HAT não é uma correção suave do conteúdo meteorológico; é um operador de seleção que praticamente remove BA-N/NE e grande parte do setor equatorial. A alta correlação do índice na grade esconde mudança grande no produto de decisão municipal. O top-10 inteiramente ao sul de 20°S não pode ser interpretado isoladamente como melhora física, porque é obtido com 248 pontos e 96 municípios zerados. Isso é **exclusão implícita de domínio**, agora demonstrada também após severidade, normalização e integração de risco |
| **Alterações implementadas** | Apenas braço comparativo em caminhos novos. `detection_mhws.py`, `hazard_index.py`, catálogo e índice vigentes, site e figuras do artigo permanecem intocados. HAT **não foi adotado** |
...
### 2026-07-30 — Execução da adoção do portão HAT e recalibração do Step 2e

| Campo | Conteúdo |
|-------|----------|
| **Natureza da entrada** | **Registro de execução.** A decisão está na entrada anterior; aqui estão os números que ela produziu. Executada por Claude Opus 5 sob instrução do pesquisador responsável, **Danilo Couto de Souza** |
| **Pergunta testada** | Executada a adoção do HAT como portão e datum, e refeita a calibração do Step 2e sobre o detector novo, o que acontece com as três consequências falsificáveis fixadas na entrada de decisão? |
| **Registro obrigatório de dissenso** | **A adoção ocorreu com o critério (c) reprovado e contra a conclusão explícita do §6 do README da comparação**, que diz: "Estes resultados não autorizam adotar HAT como método vigente". A mesma entrada de decisão determinava que, se (c) falhasse, "a decisão deve ser reaberta". A decisão não foi reaberta: foi reafirmada pelo pesquisador responsável, **Danilo Couto de Souza**, e executada assim. Nenhum texto de auditoria anterior foi apagado, suavizado ou reescrito |
| **Dados e métodos** | Cadeia completa reexecutada. Instantâneos versionados dos dois legados criados **antes** de qualquer escrita (`outputs/legacy_mhws_method/`, `outputs/legacy_threshold_calibration_ssh_total/`), porque `outputs/storm_catalog/`, `outputs/risk_index/` e `outputs/threshold_calibration/` estão no `.gitignore`. O `compound_metrics_mhws.csv` já não existia em disco e foi regenerado antes de tudo, reproduzindo o legado publicado exatamente. Step 2e recalibrado sobre o detector de produção, com grade estendida a q95 e q99. Step 3.2 regenerado com o par novo. Índice, exposição e risco municipal derivados pela mesma `derive_native_hazard_index()`, sem alteração da matemática do índice |
| **Scripts executados** | `python -m src.compound_detection.detection_mhws` (regeneração do legado); `python -m src.compound_detection.detection_hat --acceptance-arm` (teste de aceitação); `python src/02_threshold_calibration/05_pu_composite_calibration/main.py --all --workers 16`; `python -m src.compound_detection.detection_hat --workers 60`; exportadores do site; `python -m src.exploratory.audit_AUD_01_final_criteria`; `python -m src.exploratory.audit_AUD_01_step2e_score_surface`; `python -m src.exploratory.audit_AUD_02_threshold_grid_floor` |
| **Teste de aceitação, antes de mudar qualquer coisa** | O braço HAT com o par vigente q90/q90 reproduz a comparação publicada nos **oito** valores exigidos: 37.225 eventos no domínio; 248 de 808 pontos sem evento; 96 municípios sem evento; ρ(\|lat\|, Índice) = 0,658; ρ(\|lat\|, Severidade) = 0,653; Spearman do risco municipal MHWS×HAT = 0,700; top-10 ao norte de 20°S = 0%; `thr_hs` recomputado idêntico ao de produção em **808/808** pontos. A regeneração do legado MHWS também reproduziu exatamente 79.639 eventos, 30.117 rejeições e igualdade em 808/808 pontos para contagem, severidade integrada, `thr_hs_abs`, duração média e intensidade de pico |
| **Achado 1 — o score composto não tem ótimo interior** | Sob o detector correto e a grade estendida, **Spearman(Score, episódios aceitos) = −0,999**: o score é uma preferência monótona por detectar menos. Seu ótimo era **q99/q99**, com **40 episódios aceitos** em SC contra 147 positivos e recall **0,034**, *menor* que o do q90/q90 (0,190). O termo dominante era `−w3·F_soft/P`, que chegava a −29,4 contra um máximo de +0,60 do termo de recall. O q90/q90 só parecia ótimo porque a grade parava nele — sinal que **AUD-02 §4** já registrava: "o ótimo está na borda da grade, o que é em si um sinal de que o ótimo pode estar fora dela". Superfície completa em `outputs/audit/AUD-01_step2e_score_surface/` |
| **Achado 2 — o peso da evidência externa media a escassez do registro, não a implausibilidade** | `E_i = 1` em **154 de 436.352** episódios não casados (**0,04 %**). Com `α_E = 0,60`, o peso de confiança `q_i` ficava limitado a **0,40 por construção** em 99,96 % dos episódios, de modo que toda detecção não casada carregava penalidade de pelo menos 0,60 independentemente de sua plausibilidade física. Isso contradiz a premissa do próprio arcabouço PU, e AUD-18 registra que não existe base independente para medir a subnotificação |
| **Mudanças de critério, autorizadas explicitamente** | Três, decididas pelo pesquisador responsável após apresentação dos números acima. **(1)** `B` deixa de ser teto unilateral `min(1, taxa/alvo)` e passa a desvio bilateral `min(1, \|log10(taxa/alvo)\|)`, ancorado na taxa reportada de Leal et al. (2024). O teto antigo era minimizado em **zero** detecções, logo empurrava na mesma direção da penalidade branda: varrer `w2` de 0,20 a 0,69 **não movia o ótimo**. **(2)** Pesos `0,60/0,20/0,20 → 0,30/0,60/0,10`, para que o único termo ancorado em observação carregue a decisão. **(3)** Alphas `0,60/0,30/0,10 → 0,20/0,50/0,30`. A janela de casamento, a auditoria de episódios e a forma do score permanecem intocadas |
| **Ancoragem da taxa, e o que ela assume** | Leal et al. (2024) registram 72 desastres costeiros declarados distintos em SC entre 1998 e 2023, isto é 2,77 eventos/ano para a costa inteira. Com a base documentária expandida, o conjunto positivo tem 147 pares (município, data) em 22,4 anos e 27 municípios, ou **0,243 detecções/município/ano**. Isso é **piso, não expectativa**: a subnotificação é a premissa do arcabouço PU. O alvo adotado, **2,0/município/ano**, assume subnotificação de ~8×. A sensibilidade cobre 0,5 a 6,0, e a seleção é estável em q70/q99 para alvo ≥ 2,0 |
| **Par selecionado, e sua comparação com q90/q90 sob o MESMO detector** | **q70/q99**. Lado a lado, em `tab_TC5_selected_vs_incumbent.csv`: recall **idêntico** (H = 28 de 147, `R_pos` = 0,1905 nos dois); `B` 0,148 contra 0,268; `F_soft` 420,4 contra 1.142,0; Score **−0,318** contra **−0,881**; episódios aceitos 484 contra 1.224; taxa detectada 1,42 contra 3,71 detecções/município/ano, para um alvo de 2,0. **O par novo não vence por detectar menos eventos reportados — vence por detectar 62 % menos ruído com o mesmo recall.** Não é degenerado: 484 episódios sobre 12 de 12 pontos de SC, 3,29 episódios por evento positivo |
| **Robustez da seleção** | `q_zos = q99` é selecionado em **14 de 14** variantes de sensibilidade (pesos, alphas, `B_target`, gap). O percentil de onda é menos estável: q70 em 8, q50 em 2, q85 em 2, q95 em 1, q99 em 1. Com os pesos antigos aplicados ao `B` bilateral novo o ótimo volta a q99/q99, o que confirma que a mudança de pesos era necessária, não cosmética. A sensibilidade de gap seleciona q70/q99 em 3 dos 4 valores; em gap = 3 seleciona q55/q99, com score praticamente empatado (−0,3176 contra −0,3178) |
| **Achado 3 — as três consequências falsificáveis, com o catálogo definitivo** | Os dois conjuntos de números, porque a comparação publicada foi feita com o par antigo e a recalibração mudou o catálogo inteiro. **(a) PASSOU, e mais forte:** ρ(\|lat\|, `Hazard_Severity`) 0,345 → 0,653 (q90/q90) → **0,699** (definitivo); AP cai de 0,429 para 0,112 e então **0,048**, contra 0,486 de SC/PR — o empate desaparece. **(b) PASSOU, e mais forte:** ρ(\|lat\|, `Hazard_Index`) 0,584 → 0,658 → **0,710**; o RS recupera 0,822 no catálogo definitivo, contra 0,664 no braço q90/q90, isto é, o par novo devolve a amplitude do Sul que o braço anterior comprimira. **(c) FALHOU:** Spearman do risco municipal no domínio 0,700 (q90/q90) → **0,695**; sobreposição do top-10 **4 de 10** nos dois; deslocamento absoluto mediano no Sul/Sudeste de 35 → **36 posições** em 99 municípios |
| **Nuance sobre (c), registrada sem alterar o veredito** | A comparação publicada avaliou (c) apenas pelo Spearman do **domínio**. Medido como o critério está redigido — "o ranking municipal estável no Sul e no Sudeste" —, o Spearman **restrito ao Sul/Sudeste** é **0,955** (era 0,971 no braço q90/q90). O valor do domínio, 0,695, é puxado pelo Norte, que é justamente onde o método remove eventos de propósito. **Isso não reabilita (c):** o topo do ranking se reorganiza (4 de 10 mantidos) e o deslocamento mediano no S/SE é de 36 posições. O critério continua **reprovado**, agora com a distinção explícita entre "o Sul/Sudeste manteve a ordenação relativa" e "o topo do ranking não se moveu", que a redação original não separava |
| **Cobertura final** | 16.768 eventos no domínio (contra 79.639 do MHWS e 37.225 do braço q90/q90); **208 de 808 pontos sem evento** (contra 248 do braço q90/q90 e 0 do MHWS); **83 de 280 municípios** associados a ponto sem evento (contra 96). A cobertura **melhora** frente ao braço q90/q90 porque o limiar de onda mais baixo compensa o portão de nível mais estrito. Política de ponto sem evento mantida em toda a cadeia: frequência 0 e severidade 0, preservando os 808 pontos na normalização |
| **Mudanças no ranking municipal** | Top-10 definitivo: São José do Norte/RS, Magé/RJ, Tavares/RS, Maricá/RJ, Mostardas/RS, Duque de Caxias/RJ, Saquarema/RJ, São Sebastião/SP, Paraty/RJ, Bertioga/SP — **0% ao norte de 20°S**, contra 50% sob MHWS. Maiores subidas: Presidente Kennedy/ES 236º→112º, Terra de Areia/RS 242º→126º, Piúma/ES 213º→100º, Itajaí/SC 230º→117º, Navegantes/SC 216º→107º. Maiores quedas: Apicum-Açu/MA 1º→158º, Vigia/PA 15º→185º, Macapá/AP 16º→172º, Colares/PA 19º→188º, São Caetano de Odivelas/PA 29º→207º |
| **Efeito sobre AUD-02** | **Piora.** O `thr_hs` mínimo nos 808 pontos cai de 0,20 m (q90) para **0,14 m** (q70); pontos abaixo de 1,0 m sobem de 35 para **56**; abaixo de 1,5 m, de 129 para **256**. Estender a grade a q99 *elevaria* o piso (0,27 m; 21 e 61 pontos), mas o par selecionado usa q99 no **nível**, não na onda. A consequência foi apresentada ao pesquisador responsável antes da execução e ele optou por seguir com q70/q99. Ver a nota correspondente em AUD-02 §14 e `outputs/audit/AUD-02_threshold_grid_floor/` |
| **Alterações implementadas** | Método HAT adotado como canônico, com o par recalibrado. `detection_mhws.py` **não foi alterado**; o detector parametrizado vive em `compound_core.py` e sua igualdade com o detector congelado é verificada em q90/q90 a cada execução de produção. Legados preservados e versionados. Site e figuras do artigo regenerados |
| **Validação realizada** | (1) Teste de aceitação nos oito valores publicados, antes de qualquer mudança de par. (2) Regeneração do legado MHWS conferida contra `hazard_by_point.csv` publicado: igualdade exata em 808/808 pontos. (3) `compound_core` idêntico ao detector congelado em q90/q90, campo a campo, incluindo dtype e shape dos arrays. (4) Fases 1 e 2 idênticas em serial e paralelo nos 30 pontos de teste. (5) Índice derivado pela mesma função e sobre a mesma população de 808 pontos nos três braços. (6) `audit_AUD_02_threshold_grid_floor` reproduz exatamente os valores publicados em AUD-02 §3 no q90 |
| **Incerteza remanescente** | (1) **AUD-02 piora**, como registrado acima, e continua bloqueando publicação. (2) O HAT segue sendo máximo amostral dependente da janela 1993–2025, não transferível a projeções. (3) O alvo de 2,0 detecções/município/ano é uma **hipótese declarada de subnotificação de ~8×**, não uma medida; AUD-18 permanece aberta. (4) O percentil de onda é o eixo mal determinado da seleção: os seis melhores pares diferem em menos de 1 % no score e cobrem de q50 a q80. (5) O teste de Rayleigh continua sem ser reexecutado sobre o catálogo final. (6) **Steps 3.1 e 3.3–3.8 não foram regenerados** — leem os catálogos de `SSH_total` do Step 3.1 e produziriam um híbrido incoerente com o método novo; ver §12. (7) AUD-04 e AUD-12 permanecem fora do escopo |
| **Próxima decisão necessária** | Duas. **(a)** Como tratar AUD-02 sob o par novo, já que a recalibração a agravou. **(b)** Regenerar os Steps 3.1 e 3.3–3.8 sobre as variáveis do método novo (`Hs` em q70 e `zos` em q99, em vez de `Hs` e `SSH_total` em q90), o que exige alterar `01_storm_catalogs/main.py` e persistir um catálogo de eventos compostos a partir do detector HAT |
