...
| Field               | Value                                          |
|---------------------|------------------------------------------------|
| Source              | Derived from GLORYS12 + WAVERYS (see below)    |
| Variables           | `zos` — sea surface height (m, GLORYS interpolated) · `VHM0` — daily Hs (m) · `VMDR` — daily wave direction (°) |
| Temporal resolution | Daily                                          |
| Period              | 1993-01-01 to 2025-12-31                       |
| Grid                | 10 latitude × 11 longitude points (~0.2° spacing, WAVERYS grid) |
| Purpose             | Unified input for compound coastal-event analysis |

This file is generated by `src/preprocessing/interpolate_glorys_to_waverys_grid.py`.
GLORYS `zos` is spatially interpolated (bilinear) onto the WAVERYS grid.
WAVERYS variables are resampled to daily means.
See `src/preprocessing/README.md` for full documentation of this step.
